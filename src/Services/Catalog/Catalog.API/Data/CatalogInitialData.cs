...
{
    // Marten support date seeding to generate inital data for the desire db
    public class CatalogInitialData : IInitialData
    {
        public async Task Populate(IDocumentStore store, CancellationToken cancellation)
        {
            // create a lightweight session to store the incoming inital data
            using var session = store.LightweightSession();

            // check if any products exist in the db,
            // if yes then not proceed data seeding
